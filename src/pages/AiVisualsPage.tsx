...
import {
  AI_SERVICE_PILLARS,
  AI_SERVICE_PROCESS,
  AI_SERVICE_DELIVERABLES,
  AI_SERVICE_FAQ,
  AI_SERVICE_STORY,
} from '../data/aiVisualsService'

const CASE = PROJECTS.find((p) => p.id === 'ai-brand-visuals')!

export function AiVisualsPage() {
  ...
  return (
    <section className="relative flex min-h-[90svh] flex-col justify-end overflow-hidden pb-16 pt-32 sm:pb-24">
      <div className="absolute inset-0 -z-10">
        <img src={IMG.aiVisuals.heroBg} alt="" className="h-full w-full object-cover opacity-70" />
      ...
      <div className="mx-auto grid w-full max-w-[1400px] grid-cols-1 items-end gap-10 px-6 sm:px-10 lg:grid-cols-[1fr_380px] lg:px-16">
        <div>
          ...
          >
            Консистентный персонаж и продукт-рендеры без фотостудии: обучаю кастомную модель на
            стиле бренда — она держит лицо, свет и настроение в каждой генерации.
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        >
          <img src={IMG.aiVisuals.productRender} alt="Пример AI-рендера продукта" className="h-full w-full object-cover" />
        </motion.div>
      </div>
    ...
    <section id="story" className="scroll-mt-24 px-6 py-24 sm:px-10 lg:px-16">
      <div className="mx-auto max-w-[1400px]">
        <p className="mb-3 flex items-center gap-2 text-sm font-medium uppercase tracking-[0.2em] text-[var(--color-text-muted)]">
          <span className="h-px w-8 bg-[var(--color-accent)]" />
          От референсов до готовой модели
        </p>
        <h2 className="font-display mb-14 max-w-2xl text-4xl font-semibold tracking-tight sm:text-6xl">Как обычные фото превращаются в бесконечный банк визуалов</h2>

        <StickyMediaStory
          steps={AI_SERVICE_STORY}
          renderMedia={(_, step) => (
            <div className="aspect-[4/3] w-full max-w-[520px] overflow-hidden rounded-3xl border border-[var(--color-border)] shadow-2xl">
...
function PillarsSection({ ref }: { ref: RefObject<HTMLDivElement | null> }) {
  return (
    <section className="relative border-y border-[var(--color-border)] px-6 py-28 sm:px-10 lg:px-16">
      <SectionBackdrop src={IMG.aiVisuals.heroBg} opacity={0.1} />
      <div className="mx-auto max-w-[1400px]">
        <p className="mb-3 flex items-center gap-2 text-sm font-medium uppercase tracking-[0.2em] text-[var(--color-text-muted)]">
          <span className="h-px w-8 bg-[var(--color-accent)]" />
          Что входит
        </p>
        <h2 className="font-display max-w-2xl text-4xl font-semibold tracking-tight sm:text-6xl">От референсов до готовой модели бренда</h2>

        <div ref={ref} className="mt-16 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {AI_SERVICE_PILLARS.map(({ icon: Icon, title, description }) => (
            <div
              key={title}
      ...
      <div className="mx-auto max-w-[1400px]">
        ...
        <div className="mt-14 grid grid-cols-1 gap-10 lg:grid-cols-[420px_1fr] lg:items-start">
          ...
          <div className="space-y-8">
            ...
            {CASE.outcome && (
              <div className="flex flex-wrap gap-8 rounded-2xl border border-[var(--color-accent)]/30 bg-[var(--color-accent-soft)] p-5">
                {CASE.outcome.stats?.map((s) => (
                  <div key={s.label}>
                    <p className="font-mono-num text-2xl font-bold text-[var(--color-accent)]">{s.value}</p>
                    <p className="mt-1 max-w-[160px] text-xs text-[var(--color-text-muted)]">{s.label}</p>
                  </div>
                ))}
              </div>
            )}
  ...
  return (
    <section className="border-t border-[var(--color-border)] px-6 py-28 sm:px-10 lg:px-16">
      <div className="mx-auto max-w-[1400px]">
        ...
        <h2 className="font-display mb-16 max-w-2xl text-4xl font-semibold tracking-tight sm:text-6xl">Шесть шагов от референсов до готовой модели</h2>

        <ProcessRail steps={AI_SERVICE_PROCESS} />
      </div>
    </section>
  )
}
      ...
      <div className="relative mx-auto grid max-w-[1400px] grid-cols-1 gap-14 lg:grid-cols-2 lg:items-center">
        <div>
          ...
          <p className="mt-6 max-w-md text-base text-[var(--color-text-muted)]">
            Точный состав и стоимость считаются индивидуально — под объём визуалов и то, нужна ли
            обученная модель бренда с нуля.
          </p>
        </div>
        <ul className="space-y-3">
          {AI_SERVICE_DELIVERABLES.map((item, i) => (
            <motion.li
              key={item}
